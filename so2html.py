...
class Marker(object):
    ...
    def __str__(self):
        if self.is_end:
            ...
            return '<%s %s>' % (self.span.tag(), attributes)
...
def resolve_heights(spans):
    ...
    open_span = []
    # TODO avoid cmp_to_key
    for s in sorted(spans, key=cmp_to_key(leftmost_sort)):
        open_span = [o for o in open_span if o.end > s.start]
        open_span.append(s)
        # TODO: use a sorted container instead. TODO2: avoid cmp_to_key
        open_span.sort(key=cmp_to_key(longest_sort))
...
def coarse_type(type_):
    ...
    # Known mappings
    for prefix, value in prefix_to_coarse_type.items():
    ...
    try:
        parsed = urllib.parse.urlparse(type_)
...
def _add_formatting_spans(spans, text):
    ...
    for s in re.split('(\n)', text):
        # don't add empty, space-only, or whole-text (excluding space)
        if s and not s.isspace() and not s.strip() == text.strip():
...
def _standoff_to_html(text, standoffs, legend, tooltips, links):
    ...
    while i < len(markers):
        ...
        # matters) TODO: avoid cmp_to_key
        to_open.sort(key=cmp_to_key(marker_sort))
        to_close.sort(key=cmp_to_key(marker_sort))
    ...
    return css, ''.join(str(o) for o in out)
...
def darker_color(c, amount=0.3):
    ...
    if len(c) != 6:
        raise ValueError
    r, g, b = [int(h, 16)/255. for h in [c[0:2],c[2:4],c[4:6]]]
...
def oa_to_standoff(annotations, target_key='target'):
    ...
    for annotation in annotations:
        ...
        fragment = urllib.parse.urldefrag(target)[1]
        try:
            start_end = fragment.split('=', 1)[1]
            start, end = start_end.split(',')
        except IndexError:
            warning('failed to parse target %s' % target)
            start, end = 0, 1
...
def standoff_to_html(text, annotations, legend=False, tooltips=False,
                     links=False, complete_page=False, oa_annotations=False,
                     embeddable=False):
...
# CoNLL format support functions

def _span_ends(type_, tag):
    if type_ is None:
        return False
    elif tag == 'I-{}'.format(type_):
        return False
...
def _span_starts(type_, tag):
    if tag == 'O':
        ...
        assert tag == 'I-{}'.format(type_), 'internal error'
        return False


def conll_to_standoff(text, conll):
    spans = []
    offset, type_, start = 0, None, None
    for line in conll.splitlines():
        line = line.strip()
        fields = line.split()
        token, tag = fields
        if _span_ends(type_, tag):
            spans.append(Span(start, offset, type_))
            type_, start = None, None
        while offset < len(text) and text[offset].isspace():
            offset += 1
        if text[offset:offset+len(token)] != token:
            raise ValueError('text mismatch')
        if _span_starts(type_, tag):
            type_, start = tag[2:], offset
        offset += len(token)
